...
class CrossModalHCIAgent:
    ...
    def generate(self, transcript, news_context, mode="A"):
        base = f"""
你是跨模態媒體輿情分析 AI。比對【主流文字新聞】與【YouTube逐字稿】，
找出衝突點、互補點，並給出資安/隱私觀看警示。
...
def pipeline(keyword, youtube_url, progress=gr.Progress()):
    ...
    try:
        ...
        vtt   = agent.transcribe(audio_path)

        progress(0.75, desc="LLM 生成 A/B 分析報告…")
        result_A = agent.generate(vtt, news_context, mode="A")
        result_B = agent.generate(vtt, news_context, mode="B")

        meta = (
            f"**{title}**  ·  {uploader}  ·  {mins}分{secs}秒\n\n"
            f"新聞爬蟲命中 **{len(news_list)}** 筆，已同步 MongoDB。"
        )
...
with gr.Blocks(theme=THEME, css=CSS, title="NewsLens") as demo:
    ...
    with gr.Row(equal_height=False):
        ...
        with gr.Column(scale=1, min_width=300, elem_classes="panel"):
            ...
            input_keyword = gr.Textbox(
                label="輿情關鍵字",
                placeholder="例：輝達、iPhone、AI 法規",
                value="科技"
            )
            ...
            with gr.Tabs():
                with gr.TabItem("A  ·  極簡摘要模式"):
                    output_A = gr.Markdown(
                        value="*執行分析後，A 組報告將顯示於此。*\n\n此模式直接給出結論，不含查核線索，模擬低認知負荷閱讀情境。",
                        elem_classes="output-box"
                    )
                with gr.TabItem("B  ·  時間軸查核模式"):
                    output_B = gr.Markdown(
                        value="*執行分析後，B 組報告將顯示於此。*\n\n此模式附帶 VTT 時間戳記與逐字稿原文，供使用者交叉比對查核。",
                        elem_classes="output-box"
                    )

    btn.click(
        fn=pipeline,
        inputs=[input_keyword, input_url],
        outputs=[output_meta, output_A, output_B]
    )
